...
/// One row in a booth/exhibitor-scoped leaderboard — built by aggregating
/// `game_sessions` client-side (Firestore has no server-side group-by), so
/// it carries only what that view needs rather than the full profile.
class ExhibitorLeaderboardEntry {
  final String uid;
  final String displayName;
  final int points;
  final int rank;

  const ExhibitorLeaderboardEntry({
    required this.uid,
    required this.displayName,
  ...
  ExhibitorLeaderboardEntry withRank(int newRank) => ExhibitorLeaderboardEntry(
        uid: uid,
        displayName: displayName,
        points: points,
        rank: newRank,
